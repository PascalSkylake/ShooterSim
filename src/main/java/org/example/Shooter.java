...
public class Shooter {
    ...
    public ArrayList<Vector<N5>> propagateWholeTrajectory(Vector<N4> x, double t, int intervals) {
        ArrayList<Vector<N5>> out = new ArrayList<>();
        double dt = t / intervals;
        Vector<N5> state = VecBuilder.fill(x.get(0, 0), x.get(1, 0), x.get(2, 0), x.get(3, 0), dt);
        for (int i = 0; i < intervals; i++) {
            state = rkFour(state);
    ...
    public double thetaFromDistance(double x) {
        return 1.37 + (-0.7 * x) + (0.183 * x * x) + (-0.0137 * x * x * x) + (-3.08E-03 * x * x * x * x) + (6.5E-04 * x * x * x * x * x) + (-3.4E-05 * x * x * x * x * x * x);
    }
